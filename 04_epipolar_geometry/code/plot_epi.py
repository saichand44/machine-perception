...
def plot_epipolar_lines(image1, image2, uncalibrated_1, uncalibrated_2, E, K, plot=True):
    ...
    """
    # calculate the fundamental matrix for the pixels in image 1 and image 2
    # Using the equation F = K^{-T} * E * K^{-1}
    F = np.matmul(np.matmul(np.linalg.inv(np.transpose(K)), E), np.linalg.inv(K))

    # epipolar line equation in image 1
    epipolar_lines_in_1 = np.matmul(np.transpose(F), uncalibrated_2)
